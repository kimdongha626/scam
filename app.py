...
if os.path.exists(DATA_PATH):
    ...
    for col in data.columns:
        if col.lower() in ["class", "fraud", "target", "label"]:
            label_col = col
            break

    if label_col is None:
        st.error("❌ Fraud 라벨 컬럼을 찾을 수 없습니다. CSV 파일을 확인하세요.")
    else:
        # Fraud 비율 시각화
    ...
    st.error("❌ creditcard.csv 파일을 찾을 수 없습니다. Kaggle에서 다운로드 후 같은 폴더에 넣어주세요.")
